"""SWOT lake-storage connector — hermetic test of the per-lake Hydrocron path.

SWOT lake storage change has NO SYMFLUENCE native, so this is *spec-validated*:
the assertions reproduce the published Hydrocron LakeSP product spec on a
synthetic inline fixture — the km³ source unit delivered as the canonical
WATER_STORAGE ``km3`` (identity scale), the signed storage-CHANGE semantics
(negative drawdown preserved, NOT masked), the -999999999999.0 fill sentinel, the
``no_data`` time_str placeholder, the symmetric valid range, the half-open UTC
window, and the gridded reduction — with no network and no auth.
"""

from datetime import UTC, datetime

import numpy as np
import pytest

from cos.connectors.swot_lake_storage import (
    SOURCE_STORAGE_SCALE,
    SWOT_FILL_VALUE,
    VALID_STORAGE_RANGE_KM3,
    SWOTLakeStorageConnector,
)
from cos.core.exceptions import DataFormatError
from cos.core.models import KIND_UNITS, ObservationKind, ReductionSpec, SpatialReduction

# Hydrocron lake CSV: header, valid rows (km³, incl. a negative drawdown), a
# fill-sentinel row, a 'no_data' row, and an out-of-window row. storage is km³.
MOCK_CSV = """\
lake_id,time_str,storage,storage_units
6350900223,2024-01-05T12:00:00Z,1.25,km3
6350900223,2024-01-12T12:00:00Z,-0.75,km3
6350900223,2024-01-19T12:00:00Z,-999999999999.0,km3
6350900223,no_data,-999999999999.0,km3
6350900223,2025-06-01T12:00:00Z,2.00,km3
"""


def test_canonical_unit_is_km3():
    """Spec: WATER_STORAGE canonical unit is 'km3'; identity boundary scale."""
    assert KIND_UNITS[ObservationKind.WATER_STORAGE] == "km3"
    assert SWOTLakeStorageConnector.kind == ObservationKind.WATER_STORAGE
    assert SOURCE_STORAGE_SCALE == 1.0


def test_parse_identity_scale_and_window():
    """Spec: storage km³ == canonical km3 (identity); window is half-open."""
    points = SWOTLakeStorageConnector.parse_timeseries(
        MOCK_CSV,
        datetime(2024, 1, 1, tzinfo=UTC), datetime(2025, 1, 1, tzinfo=UTC),
    )
    by_date = {p.timestamp.date().isoformat(): p for p in points}
    # 2025-06-01 is outside [start, end); 'no_data' row is dropped (no timestamp).
    assert "2025-06-01" not in by_date
    assert "no_data" not in by_date
    # Identity scale: km³ value passes through unchanged.
    assert by_date["2024-01-05"].value == pytest.approx(1.25)
    assert by_date["2024-01-05"].quality.value == "good"


def test_negative_storage_change_preserved():
    """Spec: storage is a SIGNED change — negative drawdown is physical, not masked."""
    points = SWOTLakeStorageConnector.parse_timeseries(
        MOCK_CSV,
        datetime(2024, 1, 1, tzinfo=UTC), datetime(2025, 1, 1, tzinfo=UTC),
    )
    by_date = {p.timestamp.date().isoformat(): p for p in points}
    assert by_date["2024-01-12"].value == pytest.approx(-0.75)
    assert by_date["2024-01-12"].quality.value == "good"


def test_fill_sentinel_maps_to_missing():
    """Spec: -999999999999.0 is the SWOT no-observation fill -> MISSING/None."""
    assert SWOT_FILL_VALUE == -999999999999.0
    points = SWOTLakeStorageConnector.parse_timeseries(
        MOCK_CSV,
        datetime(2024, 1, 1, tzinfo=UTC), datetime(2025, 1, 1, tzinfo=UTC),
    )
    by_date = {p.timestamp.date().isoformat(): p for p in points}
    assert by_date["2024-01-19"].value is None
    assert by_date["2024-01-19"].quality.value == "missing"


def test_no_data_time_str_is_dropped():
    """A 'no_data' time_str placeholder has no anchor timestamp and is skipped."""
    points = SWOTLakeStorageConnector.parse_timeseries(
        MOCK_CSV,
        datetime(2024, 1, 1, tzinfo=UTC), datetime(2025, 1, 1, tzinfo=UTC),
    )
    # 2 valid + 1 fill (still inside window) = 3 anchored points; 'no_data' and
    # the out-of-window row are absent.
    assert len(points) == 3


def test_out_of_range_storage_masked_to_missing():
    """Spec: a finite storage outside the symmetric band -> MISSING."""
    lo, hi = VALID_STORAGE_RANGE_KM3
    csv_text = (
        "lake_id,time_str,storage\n"
        f"1,2024-02-01T00:00:00Z,{hi * 10:.1f}\n"   # absurdly large +ve -> out of range
        f"1,2024-02-02T00:00:00Z,{lo * 10:.1f}\n"   # absurdly large -ve -> out of range
        "1,2024-02-03T00:00:00Z,3.0\n"              # plausible -> good
    )
    assert lo == -hi  # symmetric about zero (signed change)
    points = SWOTLakeStorageConnector.parse_timeseries(
        csv_text, datetime(2024, 1, 1, tzinfo=UTC), datetime(2025, 1, 1, tzinfo=UTC),
    )
    by_date = {p.timestamp.date().isoformat(): p for p in points}
    assert by_date["2024-02-01"].value is None
    assert by_date["2024-02-01"].quality.value == "missing"
    assert by_date["2024-02-02"].value is None
    assert by_date["2024-02-02"].quality.value == "missing"
    assert by_date["2024-02-03"].value == pytest.approx(3.0)


def test_non_km3_units_rejected():
    """Spec contract: a storage_units other than km³ must not be mis-scaled."""
    csv_text = (
        "lake_id,time_str,storage,storage_units\n"
        "1,2024-02-01T00:00:00Z,1.25,m3\n"
    )
    with pytest.raises(DataFormatError):
        SWOTLakeStorageConnector.parse_timeseries(
            csv_text, datetime(2024, 1, 1, tzinfo=UTC), datetime(2025, 1, 1, tzinfo=UTC),
        )


def test_missing_required_column_raises():
    with pytest.raises(DataFormatError):
        SWOTLakeStorageConnector.parse_timeseries(
            "lake_id,storage\n1,1.25\n",  # no time_str column
            datetime(2024, 1, 1, tzinfo=UTC), datetime(2025, 1, 1, tzinfo=UTC),
        )


def test_missing_storage_column_raises():
    with pytest.raises(DataFormatError):
        SWOTLakeStorageConnector.parse_timeseries(
            "lake_id,time_str\n1,2024-02-01T00:00:00Z\n",  # no storage column
            datetime(2024, 1, 1, tzinfo=UTC), datetime(2025, 1, 1, tzinfo=UTC),
        )


def test_json_envelope_is_unwrapped():
    """Hydrocron may wrap the CSV in {'results': {'csv': ...}}; parser unwraps it."""
    import json

    body = json.dumps({"status": "200 OK", "results": {"csv": MOCK_CSV}})
    points = SWOTLakeStorageConnector.parse_timeseries(
        body, datetime(2024, 1, 1, tzinfo=UTC), datetime(2025, 1, 1, tzinfo=UTC),
    )
    assert len(points) == 3
    assert points[0].value == pytest.approx(1.25)


def test_column_order_independent():
    """Columns are matched by header name, not position."""
    csv_text = (
        "time_str,storage,lake_id\n"
        "2024-03-01T00:00:00Z,0.5,1\n"
    )
    points = SWOTLakeStorageConnector.parse_timeseries(
        csv_text, datetime(2024, 1, 1, tzinfo=UTC), datetime(2025, 1, 1, tzinfo=UTC),
    )
    assert len(points) == 1
    assert points[0].value == pytest.approx(0.5)


@pytest.mark.asyncio
async def test_fetch_series_builds_lake_series(monkeypatch):
    conn = SWOTLakeStorageConnector()

    async def _fake_fetch(self, feature, feature_id, start, end):  # noqa: ANN001
        assert feature == "PriorLake"
        assert feature_id == "6350900223"
        return MOCK_CSV

    monkeypatch.setattr(SWOTLakeStorageConnector, "_fetch_timeseries", _fake_fetch)
    spec = ReductionSpec(domain_name="lakes", station_ids=("swot:6350900223",))
    series_list = await conn.fetch_series(
        spec, datetime(2024, 1, 1, tzinfo=UTC), datetime(2025, 1, 1, tzinfo=UTC)
    )
    assert len(series_list) == 1
    s = series_list[0]
    assert s.kind == ObservationKind.WATER_STORAGE
    assert s.unit == "km3"
    assert s.reduction == SpatialReduction.STATION
    assert s.site.kind == "station"
    assert s.site.site_id == "swot:6350900223"
    good = [p for p in s.points if p.value is not None]
    assert len(good) == 2
    # Both the positive (1.25) and the negative (-0.75) change survive.
    assert sorted(p.value for p in good) == pytest.approx([-0.75, 1.25])


@pytest.mark.asyncio
async def test_list_sites_from_explicit_ids():
    conn = SWOTLakeStorageConnector()
    spec = ReductionSpec(domain_name="x", station_ids=("6350900223", "swot:9999999999"))
    sites = await conn.list_sites(spec)
    assert {s.site_id for s in sites} == {"swot:6350900223", "swot:9999999999"}
    assert all(s.kind == "station" for s in sites)


# -- gridded path (synthetic storage NetCDF) ---------------------------------


@pytest.fixture
def swot_storage_nc(tmp_path):
    """A synthetic SWOT-like lake-storage NetCDF (km³), with a fill cell to mask."""
    xr = pytest.importorskip("xarray")
    pytest.importorskip("netCDF4")
    times = np.array(["2024-06-15", "2024-07-15"], dtype="datetime64[ns]")
    lats = np.array([50.0, 51.0, 52.0])
    lons = np.array([-116.0, -115.0, -114.0])
    data = np.full((2, 3, 3), 2.0)  # km³
    data[0, 0, 0] = SWOT_FILL_VALUE  # one fill cell -> masked to NaN before mean
    ds = xr.Dataset(
        {"storage": (("time", "lat", "lon"), data)},
        coords={"time": times, "lat": lats, "lon": lons},
    )
    path = tmp_path / "swot_storage_synth.nc"
    ds.to_netcdf(path)
    return path


def test_reduce_file_basin_mean_identity(swot_storage_nc):
    conn = SWOTLakeStorageConnector()
    spec = ReductionSpec(
        domain_name="bow", bbox=(50.0, -116.0, 52.0, -114.0),
        centroid=(51.0, -115.0), area_km2=8000.0,  # large -> basin_mean
    )
    series = conn.reduce_file(
        swot_storage_nc, spec,
        datetime(2024, 1, 1, tzinfo=UTC), datetime(2025, 1, 1, tzinfo=UTC),
    )
    assert series.kind == ObservationKind.WATER_STORAGE
    assert series.unit == "km3"
    assert series.reduction == SpatialReduction.BASIN_MEAN
    assert series.site.kind == "reduced_region"
    # Valid cells are 2 km³ (fill cell masked); identity scale -> 2 km³.
    for p in series.points:
        assert p.value == pytest.approx(2.0, abs=1e-6)
        assert p.quality.value == "good"


def test_reduce_file_window_trim_half_open(swot_storage_nc):
    conn = SWOTLakeStorageConnector()
    spec = ReductionSpec(domain_name="bow", bbox=(50.0, -116.0, 52.0, -114.0),
                         centroid=(51.0, -115.0), area_km2=8000.0)
    # Half-open [2024-06-01, 2024-07-15): includes 06-15, excludes 07-15.
    series = conn.reduce_file(
        swot_storage_nc, spec,
        datetime(2024, 6, 1, tzinfo=UTC), datetime(2024, 7, 15, tzinfo=UTC),
    )
    months = {(p.timestamp.year, p.timestamp.month) for p in series.points}
    assert (2024, 6) in months
    assert (2024, 7) not in months


@pytest.mark.network
@pytest.mark.asyncio
async def test_live_smoke_swot_lake_storage():
    """LIVE smoke against the real anonymous Hydrocron endpoint.

    Run with: pytest -m network tests/connectors/test_swot_lake_storage.py -k live
    """
    conn = SWOTLakeStorageConnector()
    spec = ReductionSpec(domain_name="lakes", station_ids=("6350900223",))
    async with conn:
        series_list = await conn.fetch_series(
            spec, datetime(2024, 1, 1, tzinfo=UTC), datetime(2024, 12, 31, tzinfo=UTC)
        )
    assert series_list and series_list[0].unit == "km3"
